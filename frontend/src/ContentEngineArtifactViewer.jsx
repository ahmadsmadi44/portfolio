...
export default function ContentEngineArtifactViewer({ item, onClose }) {
  ...
  const move=direction=>setIndex(value=>(value+direction+collection.length)%collection.length);
  ...
  return <dialog ref={dialog} className="artifact-dialog" aria-labelledby="artifact-title" onCancel={onClose} onClick={e=>{if(e.target===e.currentTarget)onClose();}} onKeyDown={e=>{if(collection && !['INPUT','TEXTAREA'].includes(e.target.tagName)){if(e.key==='ArrowRight'){e.preventDefault();move(1);}if(e.key==='ArrowLeft'){e.preventDefault();move(-1);}}}}>
    ...
    <div className="artifact-body">
      {collection && <nav className="artifact-navigation" aria-label="Browse deliverable"><button onClick={()=>move(-1)} aria-label="Previous item"><ChevronLeft size={20}/></button><span aria-live="polite">{code==='LI'?'Post':code==='CAR'?'Slide':'Image'} {index+1} / {collection.length}</span><button onClick={()=>move(1)} aria-label="Next item"><ChevronRight size={20}/></button></nav>}
      {code==='CAR' && <><div className="artifact-carousel"><img src={collection[index].src} alt={collection[index].alt}/><div className="artifact-slide-copy"><span>north loam coffee co.</span><h3>{collection[index].copy}</h3><small>{String(index+1).padStart(2,'0')} / 08</small></div></div><p className="artifact-note">Instagram carousel · supplied slide copy paired with its matching generated image. Use the arrows or your keyboard to browse.</p></>}
