...
export default function ContentEngineDemo() {
  ...
  return <section className="ce-run" aria-labelledby="ce-run-heading">
    ...
    <div className="ce-console">
      <div className="ce-windowbar"><span><i/><i/><i/> runs/north_loam/device_check</span><span>LANGGRAPH · DEMO MODE</span></div>
      ...
      <div className="ce-run-grid">
        <div className="ce-graph-panel">
          <div className="ce-panel-head"><span>workflow/content_engine.workflow.json</span><span>48 nodes</span></div>
          <div className="ce-node-grid">{runNodes.map((node,index) => { const Icon=iconFor(node.type); const state=index < step ? 'done' : index === step ? 'active' : 'waiting'; return <div className="ce-run-node" data-state={state} key={node.id}><span className="ce-node-type">{node.type}</span><Icon size={15}/><div><strong>{node.title}</strong><small>{node.detail}</small></div><b>{state === 'done' ? '✓' : state === 'active' ? 'RUN' : node.ms ? `${node.ms} ms` : 'WAIT'}</b></div>})}</div>
          <div className="ce-run-status"><i data-running={running}/>{finished ? 'Approved · 13 deliverables recorded' : step >= 0 ? `Running: ${runNodes[Math.min(step, runNodes.length - 1)].id}` : 'Ready: founder interview loaded'}</div>
        </div>
      ...
      <div className="ce-reliability"><div><span className="ce-kicker">RELIABILITY & OPERATIONS</span><h3>Designed to keep running after the demo.</h3></div>{reliabilityFacts.map(f => <article key={f.title}><strong>{f.title}</strong><p>{f.body}</p><small>{f.verifiedBy}</small></article>)}</div>
    </div>
  </section>;
}
